class_name Groups
## Purpose: Defines node group constants.

# Binding consumers
const RUN_STATE_CONSUMERS := &"run_state_consumers"
const COMBATANT_SYSTEM_CONSUMERS := &"combatant_system_consumers"
# Scene instance groups
const LOOT := &"loot"
const INTERACTABLES := &"interactables"
const COMBATANTS := &"combatants"
const COLLECTORS := &"resource_collectors"
const RANGED_ATTACK := &"ranged_attack"
const PLAYER_SPAWNS := &"player_spawns"
const ENEMY1_SPAWNS := &"enemy1_spawns"
const ENEMY2_SPAWNS := &"enemy2_spawns"
# Debug
const DEBUG_OVERLAY_SQUAD := &"debug_overlay_squad"
const DEBUG_OVERLAY_COMBATANT := &"debug_overlay_combatant"
const DEBUG_OVERLAY_NAV := &"debug_overlay_navigation"
const DEBUG_OVERLAY_SELECTION := &"debug_overlay_selection"
const DEBUG_OVERLAY_HEATMAP := &"debug_overlay_heatmap"
const DEBUG_OVERLAY_BELIEF := &"debug_overlay_belief"
