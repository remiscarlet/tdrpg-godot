...
func _pick_best_open_task(hauler: Node2D) -> HaulTask:
    ...
    for t in _tasks:
        ...
        var score: float = (
            hauler.global_position.distance_to(loot.global_position)
...
func _prune_invalid_tasks() -> void:
    _tasks = _tasks.filter(
        func(t: HaulTask) -> bool:
            var loot := instance_from_id(t.loot_id)
            var col := instance_from_id(t.collector_id)
            return (
                is_instance_valid(loot)
                and is_instance_valid(col)
                and t.status != HaulTask.Status.DONE
            )
    )
