...
## Asserts layer bitmask creation combines the provided layer indices.
func test_as_mask_combines_layers() -> void:
    var mask := PhysicsUtils.as_mask(PackedInt32Array([1, 3]))
    assert_int(mask).is_equal(0b101)


## Ensures world-collidable layers produce the expected mask value.
...
## Verifies player projectiles use correct hit layer and mask against enemy hurtboxes.
